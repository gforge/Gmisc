...
prComputeFanCenterBus <- function(starts, end, subelmnt, margin = unit(2, "mm"), pad_mm = unit(1, "mm")) {
    ...
    # Get actual start x positions (not evenly-spaced slots)
    starts_x_vals <- vapply(s_coords, function(s) {
        u <- x_at(s)
    ...
    # Get end box center for trunk placement
    end_center_x <- tryCatch(convertX(e_coords$x, "npc", valueOnly = TRUE), error = function(e) {
        mean(range(starts_x_vals, na.rm = TRUE))
    })

    # Bus spans from leftmost to rightmost start box (just like one-to-many N)
...
prMakeFanCenterStems <- function(starts, end, bend_y, starts_above, subelmnt, lty_gp) {
    ...
    lapply(seq_along(s_coords), function(i) {
        s <- s_coords[[i]]
        x0 <- x_at(s)
        if (!inherits(x0, "unit")) x0 <- unit(x0, "npc") else x0 <- convertX(x0, "npc")
