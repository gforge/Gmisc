...
#'
#' Sets selected boxes to a shared height. This is useful when boxes in the
#' same flowchart row or group should have a uniform vertical size, regardless
...
#'  among selected boxes is used.
#'
#' @return The updated object with equalized heights.
#' @export
#' @family flowchart components
#' @examples
#' fc <- flowchart(
...
#'
#' fc |>
#'   equalizeHeights(subelement = "groups")
#'
#' # Global fixed height via explicit argument
#' fc |> equalizeHeights(subelement = "groups", height = grid::unit(20, "mm"))
#' @md
equalizeHeights <- function(x, subelement = NULL, height = NULL) {
  if (inherits(x, "box")) {
    if (is.null(height)) {
      return(x)
    }
    return(prSetBoxDimensions(x, height = height))
  }
  ...
  if (!inherits(x, "Gmisc_list_of_boxes")) {
    stop("equalizeHeights() requires a box or list of boxes.", call. = FALSE)
  }

  if (!is.null(height) && is.numeric(height)) {
    height <- unit(height, "mm")
  }
  if (!is.null(height) && !inherits(height, "unit")) {
    stop("`height` must be a unit or numeric.", call. = FALSE)
  }
  ...
  if (is.null(target_height)) {
    heights_mm <- vapply(selected, function(b) {
      prConvertHeightToMm(coords(b)$height)
    }, numeric(1))
    target_height <- unit(max(heights_mm), "mm")
  }

  for (i in seq_along(selected)) {
    updated <- prSetBoxDimensions(selected[[i]], height = target_height)
    if (isTRUE(selected_meta[[i]]$first_container)) {
      x[[1]] <- set_list_element_by_path(x[[1]], selected_meta[[i]]$path, updated)
    } else {
      x <- set_list_element_by_path(x, selected_meta[[i]]$path, updated)
