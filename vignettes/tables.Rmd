...
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Tables using Gmisc and some competing alternatives}
...
As the `Gmisc`-package is all about HTML tables we will start with these.
...
The dataset has been downloaded from Statistics Sweden and is attached to the Gmisc package. We will start by reshaping our tidy dataset into a more table adapted format.
...
<table>
<tr>
  <td colspan="6" style="text-align: center">County</td></tr>
<tr>
  <td colspan="3" style="text-align: center">Men</td>
  <td colspan="3" style="text-align: center">Women</td></tr>
<tr>
...
Next step is to output the table after rounding to the correct number of decimals. The `txtRound` function helps with this, as it uses the `sprintf` function instead
of the `round` the resulting strings have the correct number of decimals, i.e. 1.02 will by round become 1 while we want it to retain the last decimal, i.e. be shown as 1.0.
