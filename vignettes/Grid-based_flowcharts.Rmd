...
```{r flowchart-example, fig.height = 7, fig.width = 8}
# Shared styling
...
Each stage's arms are spread across the viewport with a small outer `margin`,
leaving a clear central gap for the labels to sit in. Equal
...
demonstrates the `flowchart()` pipe API at full scale, using named lists for parallel
boxes, `spread()` to position each column, and pairwise `connect()` for the arms.

Key points:

- `arms`, `lost`, and `analysis` are all **named lists** — `connect("arms", "lost", ...)`
  automatically creates one connector per matching pair (same list index).
- `spread(..., exclude = "lost")` keeps side-branch boxes out of the main
  vertical spacing, and `align(..., references = list("arms", "analysis"))`
  then centres them between the rows they describe.
- `type = "L"` exits the **bottom** of the assessed box and turns right, giving the
  classic "down then right" exclusion branch instead of a direct side-exit.
- `insert()` adds the exclusion box; `move(..., just = "right")` places its
  right edge at the outer exclusion margin, and `align(..., references = ...)`
  centres it between the before/after boxes.
- Badge labels extend above the box top and are not counted in the height used by
  `spread()`, so a `margin` of 10 mm is used to leave room at the top.

```{r consort_example, fig.width = 9, fig.height = 6}
...
In time-to-event analyses, participants who leave follow-up may still
contribute information until censoring. Dotted side-entry arrows can show that
these participants return to the analysis set instead of being dropped from the
study.

```{r consort_dotted_return_arrows, fig.width = 10, fig.height = 9}
...
- reduce text padding globally for all `boxGrob()` calls
- make corresponding boxes in different levels share the same width

The snippet below demonstrates both via `options(boxGrobTxtPadding = ...)` and `equalizeWidths()`.

```{r flowchart-group-width-and-padding, fig.height = 5.5, fig.width = 8}
old_opts <- options(boxGrobTxtPadding = unit(2, "mm"))
...
The regular `vertical` and `horizontal` connectors draw a straight line between
the relevant box faces. If the box centres are offset, that straight line can be
diagonal. Use `vertical_axis` or `horizontal_axis` when the connector itself must
stay on the source box's x- or y-axis and land on the target boundary.

This is useful for shared destination boxes: each upstream item can land cleanly
on its own projected position without adding invisible helper boxes.

```{r axis_preserving_connectors, fig.width = 10, fig.height = 6}
axis_box_gp <- gpar(fill = "#F3F8FF", col = "#3B73C5", lwd = 1.3)
target_gp   <- gpar(fill = "#FFF4C7", col = "#C69214", lwd = 1.3)
axis_con_gp <- gpar(col = "#555555", fill = "#555555", lwd = 1.3)

grid.newpage()
flowchart(
  web = boxGrob("Web form", x = .18, y = .78, box_gp = axis_box_gp),
  api = boxGrob("API upload", x = .38, y = .78, box_gp = axis_box_gp),
  manual = boxGrob("Manual entry", x = .58, y = .78, box_gp = axis_box_gp),
  validation = boxGrob(
  ...
  missing = boxGrob("Missing fields", x = .12, y = .32, box_gp = axis_box_gp),
  duplicate = boxGrob("Duplicate ID", x = .12, y = .21, box_gp = axis_box_gp),
  outlier = boxGrob("Outlier value", x = .12, y = .10, box_gp = axis_box_gp),
  log = boxGrob(
    "Issue log",
    x = .72, y = .21,
    height = unit(.34, "npc"),
  ...
  connect(c("web", "api", "manual"), "validation",
          type = "vertical_axis", lty_gp = axis_con_gp, arrow_size = 3) |>
  connect(c("missing", "duplicate", "outlier"), "log",
          type = "horizontal_axis", lty_gp = axis_con_gp, arrow_size = 3) |>
  print()
```
...
```{r box_coordinates, fig.height = 3, fig.width = 4}
grid.newpage()
smpl_bx <- boxGrob(
  label = "A simple box",
  x = .5,
...
You can create alternate box shapes by passing a custom `box_fn` to `boxGrob`,
or use the convenience helpers `boxDiamondGrob()`, `boxEllipseGrob()` and
`boxRackGrob()` that ship with the package.

```{r extra_shapes, fig.height = 3, fig.width = 6}
...
Below are a few commonly-used flowchart shapes demonstrating their typical
appearance and usage.

```{r standard_shapes, fig.height = 3.5, fig.width = 8}
...
`connectGrob()` also supports connecting one box to many boxes, or many boxes to one box.
For merging many boxes into one, `type = "fan_in_top"` distributes the attachment points evenly along the top edge of the destination box, with an optional margin.

```{r connect_multi, fig.width = 4, fig.height = 4}
grid.newpage()
...
Numeric `.from`, `.to`, and `.margin` values are interpreted as proportions of the current viewport (`npc`).
If only one of `.from` or `.to` is specified, the other defaults to the full span (`0 npc` or `1 npc`).
The `.margin` argument adds padding at both ends of the span (also when using `.from`/`.to`).

```{r horizontal_spread, fig.width = 11, fig.height = 8}
...
```{r complex_nested, fig.width = 7, fig.height = 8}
...
```{r s3_api_example, fig.height=8, fig.width=6, eval=FALSE}
grid.newpage()
...
```{r complex_example, fig.height = 9, fig.width = 9}
# Define the boxes
